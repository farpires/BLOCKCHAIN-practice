...
describe('Transaction',()=>{
    ...
    describe('transacting with an amount that exceeds the balance', ()=>{
        ...
        it('does not create the transacion',()=>{
            expect(()=>{
                transaction = Transaction.create(wallet,recipientAdress, amount);
    ...
    it('inputs the balance of the wuallet',()=>{
    ...
    it('validates a valid transaction',()=>{
        ...
        const muestra = Transaction.verify(transaction);
        console.log(muestra)
    ...
    describe('and updating a transaction',()=>{
        ...
        it('subtracts the next amount from the senders wuallet',()=>{
            const output = transaction.outputs.find(({address})=>address === wallet.publicKey);
            expect(output.amount).toEqual(wallet.balance - amount - nextAmount );
        });
        it('output an amount for the  next recpient',()=>{
            const outputs = transaction.outputs.find(({address})=> address===nextRecipient);
            expect(outputs.amount).toEqual(nextAmount)
        })
    })
})
